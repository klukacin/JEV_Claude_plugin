...
// A threshold that can be switched off: empty/unset keeps the default, off/none/false/no disables.
function optionalNumber(value, fallback) {
  if (value === undefined || value === null || String(value).trim() === '') return fallback;
  if (FALSE_WORDS.has(String(value).trim().toLowerCase()) || String(value).trim().toLowerCase() === 'none') return null;
  const n = Number(value);
  return Number.isFinite(n) ? n : fallback;
}
