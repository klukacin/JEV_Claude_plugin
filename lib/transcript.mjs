...
const SYSTEM_PROMPT = /^(<(task-notification|agent-message|command-name|command-message|local-command-[\w-]+|bash-(input|stdout|stderr))\b|\[Request interrupted|This session is being continued|Stop hook feedback|Caveat: The messages below)/;

// Runner invocations recognised anywhere in a command segment.
const RUNNER = new RegExp([
  String.raw`\b(npm|pnpm|yarn|bun)\b[^|;&\n]{0,120}?\s(run\s+)?(test|t|check|verify|typecheck|type-check|lint|build|e2e|ci)(:[\w:-]+)?(\s|$)`,
...
function bashVerifies(command, edits) {
  ...
  for (const words of heads) {
    ...
    if ((base === 'npx' || base === 'bunx' || base === 'pnpm' || base === 'yarn') && next && VERIFY_TOOLS.has(next)) return true;
  ...
  const runsSomething = heads.some((words) => INTERPRETERS.has(words[0].split('/').pop()) || words[0].includes('/'));
  return runsSomething && edits.some((rel) => command.includes(rel));
}
