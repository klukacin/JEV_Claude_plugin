...
const RUNNER = new RegExp([
  String.raw`\b(npm|pnpm|yarn|bun)\b[^|;&\n]{0,120}?\s(run\s+)?(test|t|check|verify|validate|spec|typecheck|type-check|lint|build|e2e|ci)([:_-][\w:-]*)?${END}`,
...
const PACKAGE_MANAGERS = new Set([...TOOL_HOSTS, 'uv', 'poetry']);
// Tools that rewrite a file without executing it.
const FORMATTERS = new Set(['prettier', 'dprint']);

// A head that can execute a file: an interpreter, a package runner, or a path. Package management
// (`npm view x.ts`, `uv pip install x.py`) and formatters (`npx prettier --write x.ts`) do not.
function runsCode(words) {
  if (words.some((w) => FORMATTERS.has(w.split('/').pop()))) return false;
  const base = words[0].split('/').pop();
  if (PACKAGE_MANAGERS.has(base)) return !words.slice(1).some((w) => PACKAGE_COMMANDS.has(w) || w === 'pip');
  return INTERPRETERS.has(base) || words[0].includes('/');
}
...
function bashVerifies(command, edits) {
  ...
  for (const words of heads) {
    ...
    if (VERIFY_TOOLS.has(base) && !PACKAGE_COMMANDS.has(next)) return true;
    // `pnpm --filter web vitest run`, `npx nx vitest`; not `pnpm --filter web add vitest` or `npx playwright install`
    const at = words.findIndex((w, i) => i > 0 && VERIFY_TOOLS.has(w));
    if (TOOL_HOSTS.has(base) && at > 0 && !words.slice(1, at + 2).some((w) => PACKAGE_COMMANDS.has(w))) return true;
  ...
  // Running the file that was just edited (a script, a hook, a module import) exercises it. A bare
  // file name counts only as an argument of the head that runs it, so `npm i && git add main.go` does not.
  const runners = heads.filter(runsCode);
  if (!runners.length) return false;
  return edits.some((rel) => {
    if (command.includes(rel)) return true;
    const name = rel.split('/').pop();
    return runners.some((words) => words.slice(1).some((w) => w.split('/').pop() === name));
  });
}
