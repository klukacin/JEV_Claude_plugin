...
const COMMAND_FAMILIES = [
  ...
  ['redirect_outside', re(String.raw`(?:^|[^<>])(?:\d|&)?>{1,2}\|?\s*["']?%O|\btee\b%S\s["']?%O|\bgit\b%S(--output[=\s]|\s-o\s*)["']?%O`)],
  ...
  ['eval_exec', re(String.raw`\beval\b|\bexec\b|\bnohup\b|\bdisown\b|\bsetsid\b|\bos\.system\(|\bsubprocess\.|child_process|\bexecSync\(|\bspawnSync\(|\bKernel\.system\b|\b(rg|ripgrep)\b%S--pre\b|\bgit\b%S\bgrep\b%S(\s-O|--open-files-in-pager)`)],
];
...
function maskMessages(text) {
  ...
  let out = text.replace(heredocForm, (m, flag, _d, _body, offset) => (insideSpan(quotedSpans(text), offset) ? m : `${flag} "Q"`));
...
function maskSearchPatterns(text) {
  return text.split('\n').map((line) => {
    if (/\$\(|`|>|--pre\b|\s-O|--open-files-in-pager|--output/.test(line)) return line;
    const masked = maskQuotes(line);
    if (/[;&]/.test(masked)) return line;
    const stages = masked.split('|').map((s) => s.trim().replace(/^([A-Za-z_]\w*=\S*\s+)+/, ''));
...
function headSignal(words) {
  ...
  // Any script target counts, with or without an extension (`sh install`, `node x`).
  const target = args.find((a) => !a.startsWith('-') && !/^(\d|&)?[<>]/.test(a));
  if (target && !/^'Q'$|^"Q"$/.test(target)) return 'interpreter_file';
  return null;
}
