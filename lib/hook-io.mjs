...
export async function runHook(name, handler, {
  ...
  // Spec §3: the key is never logged. A logged command preview or error message can carry it
  // (`TYPESAFE_API_KEY=ts-… node scripts/status.mjs`), so every string field is redacted.
  const log = (entry) => {
    const redacted = {};
    for (const [key, value] of Object.entries(entry ?? {})) {
      redacted[key] = typeof value === 'string' ? redact(value, cfg.apiKey) : value;
    }
    appendDecisionLog(cfg.logPath, {
      ts: new Date().toISOString(), hook: name, latency_ms: Date.now() - started, ...redacted,
    });
  };
  let output = null;
