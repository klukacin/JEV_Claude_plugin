...
function segmentIsSafe(segment) {
  ...
  switch (base) {
    ...
    case 'sed': return args[0] === '-n' && args.length >= 2 && /^\d+(,\d+)?p$/.test(args[1]) && !args.some((a) => /^-i/.test(a));
    ...
    case 'npx':
      ...
      if (args[0] === 'prettier') return args.some((a) => /^--check(=|$)/.test(a) || a === '-c' || /^-c(=|$)/.test(a));
    ...
    case 'go':
      if (args[0] === 'env' && args.slice(1).some((a) => /^-(w|u)(=|$)/.test(a))) return false;
