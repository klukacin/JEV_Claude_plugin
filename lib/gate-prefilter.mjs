...
function segmentIsSafe(segment) {
  ...
  switch (base) {
    ...
    // A second operand is the output file; `-` is an operand (stdin), not a flag.
    case 'uniq': return args.filter((a) => a === '-' || !a.startsWith('-')).length <= 1;
    // Both take the option's value from the rest of the cluster or the next argv, so a
    // clustered or abbreviated form writes a file just as well: `tree -ao out.txt`,
    // `file -bC -m magic`, `file --comp -m magic`.
    case 'tree': return !args.some((a) => /^-[A-Za-z]*o/.test(a) || /^--o/.test(a));
    case 'file': return !args.some((a) => /^-[A-Za-z]*C/.test(a) || /^--c/.test(a));
    case 'npm':
